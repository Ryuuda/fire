...
(function(exports) {
    ...
    function PlayerView(settings) {
        ...
        this.render = function ($container, data, index) {
            ...
            this.videoElement.id = "vjs-player";
            ...
            this.videoElementSource = document.createElement('source');
            this.videoElementSource.id ="vjs-source";

            var dataList = new XMLHttpRequest();
            dataList.open( "GET", "https://staging.screeningnow.com/api/v1/assets/testview/" + video_data.id, false); // false for synchronous request
            dataList.send( null );
            ...
            if(resObj.success == false){
                ...
                console.log(resObj)
                var videoURL = resObj.url;
            }

            this.videoElementSource.src = videoURL;
            this.videoElementSource.type = 'application/x-mpegURL';

            this.videoElement.appendChild(this.videoElementSource);

            // this.handleClosedCaptioning(video_data.subtitlesURL);
            this.$el.append(this.videoElement);

            videojs("vjs-player").overlay({
                overlays: [{
                    content: "<h1>USER: Jim To</h1></br><h1>IP: 123.123.1.123</h1>",
                    align: "top-right",
                    showBackground: false,
                    start: 'playing',
                    end: 'pause'
            }]});
            videojs("vjs-player").watermark({
                position: 'top-left',
                image: 'assets/sample_watermark.png'
            });
            videojs("vjs-player").watermark({
